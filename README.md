...
按讚數是用一個免費、不需註冊的第三方服務（CountAPI）做「跨訪客共用計數」，所以理論上所有人看到的數字會是同一個。
但因為是免費服務，不保證 100% 穩定；如果哪天連不上，網頁會自動切換成「只記錄在你自己這台裝置」的計數方式，不會讓功能整個壞掉，只是那種情況下數字就不是全站共用的了。如果之後想換成更穩定的方案（例如自己架一個小型計數服務），再告訴我。
